...
```cpp
static inline int _ldst_memtodev(unsigned dry_run, u8 buf[], const struct _xfer_spec *pxs, int cyc)
	int off = 0;
	while (cyc--)
...
static int pl330_probe(struct amba_device *adev, const struct amba_id *id)
  ...
  |--> ret = dma_async_device_register(pd);//为每个dma通道注册生成devices, /sys/devices/dma-pl330/dma/ 目录下,但是因为没有dmaengine功能支持,没有实际意义
...
struct dma_chan *__dma_request_channel(const dma_cap_mask_t *mask, dma_filter_fn fn, void *fn_param)
  chan = private_candidate(mask, device, fn, fn_param); dma_chan_get(chan);//-->chan->device->device_alloc_chan_resources(chan);
...
dma_cookie_t dmaengine_submit(struct dma_async_tx_descriptor *desc) //一旦传输描述符准备好并且回调函数也加入后，该函数把传输描述符加入到DMA engine驱动的等待队列，但不会启动DMA操作
