...
//pthread_kill()函数可以用来检查线程是否还在
if (pthread_kill(tid,0) == ESRCH)
    printf("ID为0x%x的线程不存在或者已经退出/n",(unsigned int)tid);
```

### 3.6 signal信号处理
kill(pid, sig)的信号是发送给进程的。进程中没有屏蔽对应信号的线程会收到信号；如果主线程没有屏蔽此信号，主线程会收到信号；否则子线程会收到信号．

**外部信号**<br>
按照 POSIX, 异步(外部)信号发送到整个进程．由于Linux线程实现上的独特性, 外部信号始终发送到主线程．
**信号处理函数**<br>
信号处理函数通过sigaction设置，注意它是多个线程共有的．但是每个线程有自己的信号掩码, 线程库根据该掩码决定将信号发送到哪个线程．当多个线程在调用sigwait等待同一信号时，只有一个线程会返回，其他线程继续阻塞
**pthread_sigmask与sigprocmask**<br>
...
int sigaddset(sigset_t *set, int signo);//用来将参数signum代表的信号增加到参数set信号集。
int sigdelset(sigset_t *set, int signo);//用来将参数signum代表的信号从参数set信号集里删除。
int sigismember(const sigset_t *set, int signo);
//函数sigemptyset初始化set所指向的信号集，使其中所有信号的对应bit清零，表示该信号集不包含任何有效信号。函数sigfillset初始化set所指向的信号集，使其中所有信号的对应bit置位，表示该信号集的有效信号包括系统支持的所有信号。注意，在使用sigset_t类型的变量之前，一定要调用sigemptyset或sigfillset做初始化，使信号集处于确定的状态。
//初始化sigset_t变量之后就可以在调用sigaddset和sigdelset在该信号集中添加或删除某种有效信号。这四个函数都是成功返回0，出错返回-1。
//sigismember是一个布尔函数，用于判断一个信号集的有效信号中是否包含某种信号，若包含则返回1，不包含则返回0，出错返回-1。

int sigwait(const sigset_t *set, int *sig); //见下文描述
...
### 5.2 list队列
见头文件 `kernel/include/linux/list.h`
```cpp
 #include <list.h>
//#include <base.h>

typedef struct audio_chn {
  ...
...
typedef	struct	_apu_codec_{
 ...
 #define list_entry_offset(ptr, type, offset) ((type *)((char *)(ptr)-(offset)))
...
list_add(&chn->chn_node, &p_codec->vchanlist); //后者为链表头
list_add_tail(&chn->chn_node, &p_codec->vchanlist); //后者为链表头
list_empty(&master->queue)
master->cur_msg = list_entry(master->queue.next, struct spi_message, queue);
list_del_init(&master->cur_msg->queue);//删除并初始化一个节点
