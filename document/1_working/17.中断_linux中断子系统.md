...
void tasklet_action(struct softirq_action *a)
	local_irq_disable(); list = __this_cpu_read(tasklet_vec.head);
	__this_cpu_write(tasklet_vec.head, NULL);	__this_cpu_write(tasklet_vec.tail, &__get_cpu_var(tasklet_vec).head);
	local_irq_enable();
	while (list)
		struct tasklet_struct *t = list; list = list->next;	t->func(t->data);
		// /t->next = NULL; *__this_cpu_read(tasklet_vec.tail) = t;	__this_cpu_write(tasklet_vec.tail, &(t->next));		__raise_softirq_irqoff(TASKLET_SOFTIRQ);

void tasklet_schedule(struct tasklet_struct *t) //添加一个就绪的tasklet到链表,并raise softirq
