...
git
terminal
sudo apt-get remove --purge 软件名称  #卸载软件
...
**zim**
直接管理目录层次结构的txt文件，可以直接使用mdwiki解析，应该是最适合的方式吧．
与它相比，cherrytree只是好看一点而已，如果需要和mdwiki交互，就必须导出，而且所有的文件都在一起
而sublime，不支持
```

```

[work_space_09_27_2017](work_space/work_space_09_27_2017.md)

audiobox文档，代码，测试

pc和环境文档

[buildroot英文手册](https://buildroot.org/downloads/manual/manual.html)

redmine不按规定更新，一次扣0.5分
 1.  是否符合1/3/6机制？
 2.  是否有设定开始时间/结束时间？
 3.  从待处理到开发中或者开发中到待处理，状态改变时，需要做一下概括。
 4. 完成后及时改成已解决。
 5. 新建问题如果不在开发中，更新成待处理。
 6. 过期问题，需要重新评估时间，包括开始和结束，同时注明一下原因
 7. 开发中的问题，至少2天更新一下状态
...
- 添加新的函数指针的方式：１．在tool/ie1807be.S文件中查找函数是否存在２．tool/labels文件中添加新的函数指针３．preloader.h中struct irom_export结构体中添加新的声明
...
**各种config文件**
```
make menuconfig /*configuration written to /home/yuan/work/dev_qsdk/.config*/
...
git log --grep ipv6 /*按照已知的关键字搜索*/
git log --author yong.yan /*按照已知的作者搜索*/
